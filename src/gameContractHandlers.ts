...
import {getOrCreateUser, typedMapToString} from "./utils";
...
export function handleGenerateEvent(
    ...
    const eventActions = eventData.get("actions")!.toArray().map<string>(obj => typedMapToString(obj.toObject()))
    if (eventActions.some((value) => value.includes("GameFinished"))) {
